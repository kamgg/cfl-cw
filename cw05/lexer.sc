...
// CW05
val DOUBLE : Rexp = (OPTIONAL(CHAR('-')) ~ (DIGIT ~ STAR(DIGIT)) ~ CHAR('.') ~ PLUS(DIGIT)) | CHAR('0')
val INT : Rexp = (OPTIONAL(CHAR('-')) ~ (RANGE("123456789".toSet) ~ STAR(DIGIT))) | CHAR('0')

val COMMENT : Rexp = "//" ~ (SYM | DIGIT | " ").% ~ "\n"
val COLON : Rexp = ":"
val NONVOIDTYPE : Rexp = "Double" | "Int"
val VOID : Rexp = "Void"

// val CONSTANT : Rexp = RANGE("ABCDEFGHIJKLMNOPQRSTUVWXYZ".toSet) ~ ("_" | LETTER | DIGIT).%
val COMMA : Rexp = CHAR(',')
